    ...
    <div class="col">
        <div class="card">
            <div class="card-body">
                ...
                <a href="#showmodal{{ $loop->iteration }}" data-bs-toggle='modal'
                    class="btn btn-success rounded-pill btn-icon">
                    <span class="tf-icons bx bx-show"></span>
                </a>
                <a href="#updatemodal{{ $loop->iteration }}" data-bs-toggle='modal'
                    class="btn btn-dark rounded-pill btn-icon">
                    <span class="tf-icons bx bx-pencil"></span>
                </a>
                <form action="{{ route('gallery.video.destroy',['galerivideo'=>$item]) }}" method="post"
                    class="d-inline">
                    @csrf
                    @method('delete')
                ...
                <form action="{{ route('gallery.video.update',['galeri'=>$item]) }}" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="exampleFormControlSelect1" name="gallery_kategori">
                            ...
                            <option value="{{ $itemgaleri->id }}" @if ($itemgaleri->id == $item->kategori_galeri_id)
                                selected @endif>
...
@endforeach
@foreach ($galeri as $item)
<div class="modal fade" id="showmodal{{ $loop->iteration }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahsingleTitle">Edit Galeri</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col">
                        <label for="badan_usaha" class="form-label">Judul Galeri</label>
                        <input type="text" id="badan_usaha" name="judul_galeri_video" class="form-control"
                            value="{{ $item->judul_galeri_video }}" readonly disabled />
                    </div>
                </div>
                <div class="mb-3">
                    <label for="exampleFormControlSelect1" class="form-label">Kategori</label>
                    <select class="form-select" id="exampleFormControlSelect1" name="gallery_kategori" disabled>
                        <option selected disabled hidden>Please select....</option>
                        @forelse ($kategoriGaleri as $itemgaleri)
                        <option value="{{ $itemgaleri->id }}" @if ($itemgaleri->id == $item->kategori_galeri_id)
                            selected @endif>
                ...
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">ID Youtube Video</label>
                    <input type="text" class="form-control" id="exampleFormControlInput1" name="link"
                        value="{{ $item->link }}" readonly disabled />
                    <div class="form-text">contoh https://www.youtube.com/watch?v=qRODifJWIwE</div>
                    <div class="form-text">id video nya adalah qRODifJWIwE</div>
                </div>
            </div>
